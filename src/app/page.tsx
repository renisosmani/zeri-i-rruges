...
import { Share2, Play, Square, X, Flame, Radio, List, Trash2, MapPin, Clock, Users, MessageCircle, AlertTriangle, Car, PlayCircle } from 'lucide-react'; 
...
const getMoodStyle = (energy: number, category?: string) => {
  if (category === '🚨') return { color: '#ef4444', shadowRgb: '239, 68, 68' };
  if (category === '🚗') return { color: '#f59e0b', shadowRgb: '245, 158, 11' };
...
type Pulse = { id: string; lat: number; lng: number; energy_value: number; audio_url: string; created_at: string; category: string; respect_count: number; parent_id?: string | null; is_quick_report?: boolean; };
...
function MapEngine() {
  ...
  // SUPABASE REALTIME & INITIAL FETCH LOGIC
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (sorted.length > 0 && sorted[0][1] >= 2) {
      ...
      const currentState = `${topCity}-${sorted[0][1]}`;
      if (lastTrendingState.current !== currentState) {
        ...
        setTimeout(() => setTrendingMsg(null), 6000);
      }
    }
  }, [pulses]);

  const getGroupedPlaylist = () => {
    const list: Pulse[] = [];
    const parents = pulses.filter(p => !p.parent_id && !p.is_quick_report).sort((a, b) => b.respect_count - a.respect_count);
  ...
  const startAutoPlay = () => {
    const playlist = getGroupedPlaylist();
    ...
    handlePlayPulse(playlist[0], true);
  };
  ...
  const playNextInQueue = () => {
    ...
    if (nextPulse) {
      handlePlayPulse(nextPulse, true);
    } else {
      isAutoPlayRef.current = false;
      setIsAutoPlayingState(false);
      setActivePulse(null);
  ...
  const handleQuickReport = async (type: '🚨' | '🚗') => {
    ...
    const { data, error } = await supabase.from('pulses').insert([{ 
      lat, lng, energy_value: 1, category: type, respect_count: 0, is_quick_report: true, audio_url: '' 
    }]).select();
  ...
  const handleUploadWithCategory = async (cat: string) => {
    ...
    try {
      ...
      isGhost = true; lat = 41.3275 + (Math.random() - 0.5) * 0.1; lng = 19.8187 + (Math.random() - 0.5) * 0.1;
    }
    try {
      const ext = audioBlob.type.includes('mp4') ? 'mp4' : 'webm';
      const fileName = `${Date.now()}.${ext}`;
      const { error: storageError } = await supabase.storage.from('audio_pulses').upload(fileName, audioBlob, { contentType: audioBlob.type });
      if (storageError) { alert("Gabim ne upload!"); setUploadStep('idle'); return; }
      ...
      const { data, error: dbError } = await supabase.from('pulses').insert([{ 
        lat, lng, energy_value: peakEnergy || 0.5, audio_url: url.publicUrl, category: isGhost ? '👻' : cat, respect_count: 0, parent_id: replyTo ? replyTo.id : null 
      }]).select();
      if (data) {
    ...
    } catch (error) { alert("Problem i panjohur!"); }
  ...
  const handleGiveRespect = async (id: string) => {
    ...
    if (!error) {
      setRespectedPulses(prev => [...prev, id]);
  ...
  const { clusters, supercluster } = useSupercluster({
    points, bounds, zoom, options: { radius: 60, maxZoom: 15 } 
  });
  ...
  const lineData = {
    ...
    features: pulses.filter(p => p.parent_id).map(child => {
      ...
      return {
        ...
        geometry: { type: 'LineString' as const, coordinates: [[child.lng, child.lat], [parent.lng, parent.lat]] }
      };
    ...
    <main className="relative w-full h-[100dvh] bg-peaky-black overflow-hidden font-sans select-none">
      ...
      <Map ref={mapRef} initialViewState={{ longitude: 20.1683, latitude: 41.1533, zoom: 7, pitch: 45 }} 
        ...
        {clusters.map(cluster => {
          ...
          const mood = getMoodStyle(pulse.energy_value, pulse.category);
          return (
            <Marker key={pulse.id} longitude={longitude} latitude={latitude} anchor="bottom">
              <div className="flex flex-col items-center cursor-pointer" onClick={() => handlePlayPulse(pulse)}>
                <span className={`text-2xl mb-1 ${pulse.category === '🚨' ? 'animate-pulse' : ''}`}>{pulse.category}</span>
                  ...
                  <div key={parent.id} className="bg-peaky-charcoal/50 border border-peaky-steel/50 rounded-3xl p-4 overflow-hidden relative">
                    ...
                    <div className="flex items-center gap-4 cursor-pointer" onClick={() => handlePlayPulse(parent)}>
                      <div className="relative">
                        <span className="text-3xl drop-shadow-lg">{parent.category}</span>
                        {isKing && <span className="absolute -top-3 -right-2 text-xl drop-shadow-[0_0_10px_gold]">👑</span>}
                      </div>
                      <div className="flex-1">
                        <div className="flex justify-between items-center">
                          <h3 className="text-white font-bold flex items-center gap-1"><MapPin size={12} className="text-peaky-blood"/> {getNearestCity(parent.lat, parent.lng)}</h3>
                          <span className="text-[10px] text-gray-400 font-mono">{getTimeAgo(parent.created_at)}</span>
                        </div>
                        <div className="flex items-center gap-3 mt-1 text-xs">
                          <span className="text-peaky-gold flex items-center gap-1"><Flame size={12}/> {parent.respect_count}</span>
                          {children.length > 0 && <span className="text-blue-400 flex items-center gap-1"><MessageCircle size={12}/> {children.length} Reply</span>}
                        </div>
                      </div>
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-lg ${activePulse?.id === parent.id ? 'bg-peaky-blood text-white shadow-[0_0_15px_rgba(220,38,38,0.5)]' : 'bg-gray-800 text-gray-400'}`}>
                        {activePulse?.id === parent.id ? <span className="animate-pulse font-mono text-xs">•••</span> : <Play size={16} className="ml-1" />}
                      </div>
                    </div>

                    {children.length > 0 && (
                      <div className="mt-3 ml-4 pl-3 border-l-2 border-peaky-steel/50 space-y-2">
                        {children.map(child => (
                           <div key={child.id} onClick={() => handlePlayPulse(child)} className="flex items-center gap-3 bg-black/30 p-2 rounded-xl cursor-pointer hover:bg-black/50 transition-colors">
                              <span className="text-xl">{child.category}</span>
                              <div className="flex-1 flex justify-between items-center">
                                 <span className="text-xs text-gray-300 font-mono">Përgjigje</span>
                                 <div className="flex items-center gap-2">
                                    <span className="text-[10px] text-gray-500">{getTimeAgo(child.created_at)}</span>
                                    {activePulse?.id === child.id ? <Square size={12} className="text-peaky-blood"/> : <Play size={12} className="text-gray-500"/>}
                                 </div>
                              </div>
                           </div>
                        ))}
      ...
      <AnimatePresence>
        {activePulse && !showGlobalList && !activeCluster && (
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }}
            ...
            <button onClick={() => { if (currentAudioElement.current) currentAudioElement.current.pause(); setActivePulse(null); isAutoPlayRef.current = false; setIsAutoPlayingState(false); }} className="w-10 h-10 bg-peaky-blood text-white rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(220,38,38,0.4)]">
                  ...
                  >
                    ...
                    <div className="flex-1 z-10">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setReplyTo(p); startRecording(); setActiveCluster(null); }} className="p-1 text-blue-400 hover:text-blue-300"><MessageCircle size={14}/></button>
                      </div>
                    </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-40 w-11/12 max-w-lg flex items-end justify-between gap-3">
        ...
        <button onClick={() => setShowGlobalList(true)} 
          className="w-14 h-14 bg-peaky-charcoal border-2 border-peaky-steel hover:border-peaky-gold rounded-2xl flex flex-col items-center justify-center text-gray-400 hover:text-white shadow-[0_10px_30px_rgba(0,0,0,0.8)] transition-all">
          <List size={22} />
          <span className="text-[9px] font-bold mt-0.5 tracking-wider">TOP</span>
        </button>
        ...
        <div className="flex-1 relative">
            ...
            {replyTo && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                ...
                <div className="bg-peaky-blood text-white text-xs px-4 py-2 rounded-full flex items-center gap-2 shadow-[0_0_15px_rgba(220,38,38,0.6)]">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setReplyTo(null); stopRecording(); }} className="ml-2 bg-black/30 rounded-full p-0.5 hover:bg-black/50"><X size={12} /></button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {uploadStep === 'category' ? (
             <div className="flex justify-around bg-peaky-charcoal p-2 rounded-2xl border-2 border-peaky-steel shadow-2xl h-14 items-center">
                {['💬', '🎸', '🚨'].map(c => <button key={c} onClick={() => handleUploadWithCategory(c)} className="text-2xl hover:scale-125 transition-transform px-2">{c}</button>)}
             </div>
          ) : (
            <button onClick={isRecording ? stopRecording : startRecording} 
              className={`w-full h-14 rounded-2xl font-bold uppercase tracking-widest text-sm flex items-center justify-center gap-2 transition-all ${isRecording ? 'bg-white text-black shadow-[0_0_30px_rgba(255,255,255,0.6)]' : 'bg-peaky-blood text-white shadow-[0_10px_30px_rgba(220,38,38,0.5)] border-2 border-red-500 hover:bg-red-700'}`}>
              {isRecording ? <Square size={16} fill="black"/> : <Radio size={18} />}
              {isRecording ? 'Ndalo' : 'Lësho Zërin'}
            </button>
          )}
        </div>

        <div className="flex flex-col gap-2">
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleQuickReport('🚨')} 
            className="w-12 h-12 bg-blue-600 border-2 border-white rounded-full flex items-center justify-center text-white shadow-lg">
            <AlertTriangle size={18} />
          </motion.button>
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => handleQuickReport('🚗')} 
            className="w-12 h-12 bg-orange-500 border-2 border-white rounded-full flex items-center justify-center text-white shadow-lg">
            <Car size={18} />
          </motion.button>
        </div>
